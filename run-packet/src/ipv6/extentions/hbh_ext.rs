...
pub struct Ipv6HbhExtHeader<T> {
    buf: T,
}

impl<T: AsRef<[u8]>> Ipv6HbhExtHeader<T> {
    #[inline]
...
impl<T: AsMut<[u8]>> Ipv6HbhExtHeader<T> {
    #[inline]
...
#[derive(Debug)]
#[repr(transparent)]
pub struct Ipv6HbhExtPacket<T> {
    buf: T,
}

impl<T: Buf> Ipv6HbhExtPacket<T> {
    #[inline]
    ...
    #[inline]
    pub fn header(&self) -> Ipv6HbhExtHeader<&[u8]> {
        let header_len = usize::from(self.buf.chunk()[1]) * 8 + 8;
        Ipv6HbhExtHeader::new_unchecked(&self.buf.chunk()[..header_len])
    }
    ...
    #[inline]
    pub fn parse(buf: T) -> Result<Ipv6HbhExtPacket<T>, T> {
        if buf.chunk().len() >= 2 {
...
impl<T: PktMut> Ipv6HbhExtPacket<T> {
    #[inline]
    pub fn set_next_header(&mut self, value: IpProtocol) {
    ...
    pub fn prepend_header<HT: AsRef<[u8]>>(
        ...
        Ipv6HbhExtPacket { buf }
    }
}

impl<'a> Ipv6HbhExtPacket<Cursor<'a>> {
    #[inline]
    pub fn cursor_header(&self) -> Ipv6HbhExtHeader<&'a [u8]> {
        let header_len = self.header_len();
        let data = &self.buf.chunk_shared_lifetime()[..header_len];
        Ipv6HbhExtHeader::new_unchecked(data)
    }
...
impl<'a> Ipv6HbhExtPacket<CursorMut<'a>> {
    #[inline]
    pub fn split(self) -> (Ipv6HbhExtHeader<&'a mut [u8]>, CursorMut<'a>) {
        let header_len = self.header_len();
        let buf_mut = self.buf.chunk_mut_shared_lifetime();
        let (hdr, payload) = buf_mut.split_at_mut(header_len);
        (
            Ipv6HbhExtHeader::new_unchecked(hdr),
