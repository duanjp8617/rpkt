...
fn build_udp_manual(mbuf: &mut Mbuf) {
...
fn build_udp_offload(mbuf: &mut Mbuf) {
    unsafe { mbuf.extend(PACKET_LEN) };
    mbuf.data_mut().fill(PAYLOAD_BYTE);
...
fn build_tcp_manual(mbuf: &mut Mbuf) {
    unsafe { mbuf.extend(PACKET_LEN) };
    mbuf.data_mut().fill(PAYLOAD_BYTE);
...
fn build_tcp_offload(mbuf: &mut Mbuf) {
    unsafe { mbuf.extend(PACKET_LEN) };
    mbuf.data_mut().fill(PAYLOAD_BYTE);
...
fn build_udp_truncated_manual(mbuf: &mut Mbuf) {
    unsafe { mbuf.extend(PACKET_LEN) };
    mbuf.data_mut().fill(PAYLOAD_BYTE);
    ...
    for i in 0..THREAD_NUM as usize {
        ...
        let jh = std::thread::spawn(move || {
            ...
            while run_clone.load(Ordering::Acquire) {
                ...
                match val {
                    0 => build_udp_manual(&mut mbuf),
                    1 => build_udp_offload(&mut mbuf),
                    2 => build_tcp_manual(&mut mbuf),
                    3 => build_tcp_offload(&mut mbuf),
                    4 => build_udp_truncated_manual(&mut mbuf),
                    5 => build_udp_truncated_offload(&mut mbuf),
                    _ => panic!("impossible"),
                }
...
fn main() {
    ...
    // 0: Generate UDP packets with manual checksum computation
    // 1: Generate UDP packets with checksum offloading
    // 2: Generate TCP packets with manual checksum computation
    // 3: Generate TCP packets with checksum offloading
    // 4: Generate truncated UDP packets with manual checksum computation
    // 5: Generate truncated UDP with checksum offloading
    let val = match arg.parse::<u64>() {
        Err(_) => {
            println!("Invalid argument: {}. The argument should be 0-5", arg);
            return;
        }
        Ok(val) => {
            if val > 5 {
                println!("Invalid argument: {}. The argument should be 0-5", val);
                return;
            }
            val
        }
