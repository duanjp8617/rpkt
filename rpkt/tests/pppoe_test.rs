...
fn pppoe_session_layer_parsing_test() {
    ...
    let ethpkt = EtherPacket::parse(pbuf).unwrap();
    assert_eq!(ethpkt.ethertype(), EtherType::PPPOE_SESSION);

    let pppoe_pkt = PPPoEPacket::parse(ethpkt.payload()).unwrap();
...
fn pppoe_session_layer_creation_test() {
    ...
    pbuf.advance(ETHER_HEADER_LEN + PPPOE_HEADER_LEN + 2);
    pbuf.chunk_mut()
        .copy_from_slice(&packet[ETHER_HEADER_LEN + PPPOE_HEADER_LEN + 2..]);

    let pbuf = PPPoEPacket::prepend_session_payload_type(pbuf, 0x0057);

    let mut pppoe_pkt = PPPoEPacket::prepend_header(pbuf, &PPPOE_HEADER_TEMPLATE);
    ...
    let mut eth_pkt = EtherPacket::prepend_header(pppoe_pkt.release(), &ETHER_HEADER_TEMPLATE);
...
fn pppoe_discovery_layer_parsing_test1() {
    ...
    let eth_pkt = EtherPacket::parse(pbuf).unwrap();
    assert_eq!(eth_pkt.ethertype(), EtherType::PPPOE_DISCOVERY);

    let pppoe_pkt = PPPoEPacket::parse(eth_pkt.payload()).unwrap();
...
fn pppoe_discovery_layer_parsing_test2() {
    ...
    let mut iter = PPPoETagMessageIter::from_message_slice(payload.chunk());
...
fn pppoe_discovery_layer_parsing_test3() {
    ...
    let mut iter = PPPoETagMessageIterMut::from_message_slice_mut(payload.chunk_mut());
    let tag_msg = iter.next().unwrap();
    assert_eq!(tag_msg.type_(), PPPoETagType::SVC_NAME);
    assert_eq!(tag_msg.header_len(), 4);
    assert_eq!(tag_msg.option_slice().len(), 0);
...
fn pppoe_discovery_layer_creation_test() {
    ...
    let mut pppoe_pkt = PPPoEPacket::prepend_header(tag1.release(), &PPPOE_HEADER_TEMPLATE);
    assert_eq!(pppoe_pkt.version(), 1);
    assert_eq!(pppoe_pkt.type_(), 1);
