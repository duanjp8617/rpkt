...
#[allow(dead_code)]
fn build_udp_manual(payload_len: usize, mp: &Mempool) -> Mbuf {
    let v = vec![0; payload_len];

    let mut mbuf = Mbuf::from_slice(&v[..], mp).unwrap();

    let total_header_len = ETHER_HEADER_LEN + IPV4_HEADER_LEN + UDP_HEADER_LEN;
    unsafe { mbuf.extend_front(total_header_len) };
...
#[allow(dead_code)]
fn build_udp_offload(payload_len: usize, mp: &Mempool) -> Mbuf {
    let v = vec![0; payload_len];
...
#[allow(dead_code)]
fn build_tcp_manual(payload_len: usize, mp: &Mempool) -> Mbuf {
    let v = vec![0; payload_len];
...
#[allow(dead_code)]
fn build_tcp_offload(payload_len: usize, mp: &Mempool) -> Mbuf {
    let v = vec![0; payload_len];
