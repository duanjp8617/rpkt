...
fn packet_l2(buf: &[u8]) {
    ...
    let ethpkt = EtherFrame::parse_from_cursor(buf).unwrap();
...
fn packet_l3(buf: &[u8]) {
    ...
    let ippkt = Ipv4::parse_from_cursor(ethpkt.payload_as_cursor()).unwrap();
...
fn packet_l4(buf: &[u8]) {
    ...
    let udppkt = Udp::parse_from_cursor(ippkt.payload_as_cursor()).unwrap();
...
fn packet_app(buf: &[u8]) {
    ...
    let payload = udppkt.payload_as_cursor();
