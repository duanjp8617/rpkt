...
mod miri_tests {
    ...
    fn create_multiseg_mbuf_from_chainer() {
        ...
        let mut new_mbuf = Mbuf::new(2048, 128);
    ...
    fn create_multiseg_mbuf_from_slice() {
        ...
        for i in 0..buf.len() + 1 {
            let mbuf = Mbuf::from_slice(&buf[..i], 2048, 128).unwrap();
    ...
    fn truncate_multiseg_mbuf() {
        ...
        for cnt in 0..buf.len() + 1 {
            let mut mbuf = Mbuf::from_slice(&buf[..], 2048, 128).unwrap();
    ...
    fn chain_mbuf_into_multiseg_mbuf() {
        ...
        for fst_mbuf_len in 1..buf.len() {
            let mut fst_mbuf = Mbuf::from_slice(&buf[..fst_mbuf_len], 2048, 128).unwrap();
            let len_fst = fst_mbuf.pkt_len();
            let seg_num_fst = fst_mbuf.num_segs();

            let snd_mbuf = Mbuf::from_slice(&buf[fst_mbuf_len..], 2048, 128).unwrap();
